...
if ! command -v milvus >/dev/null 2>&1; then
  ...
  if ! command -v etcd >/dev/null 2>&1; then
    ...
    sudo setsid etcd --data-dir=/var/lib/etcd \
      ...
      --listen-peer-urls=http://127.0.0.1:2380 </dev/null 2>&1 | sudo tee /var/log/etcd.log >/dev/null &
  ...
  cd / && sudo setsid bash -c "MILVUSCONF=/etc/milvus/configs/ /usr/bin/milvus run standalone" </dev/null 2>&1 | sudo tee /var/log/milvus.out.log >/dev/null &
