...
if ! command -v milvus >/dev/null 2>&1; then
  ...
  if ! command -v etcd >/dev/null 2>&1; then
    ...
    # Start etcd as a background daemon using setsid for proper session detachment
  ...
  # Start Milvus manually in background using setsid for proper session detachment
  ...
  # Copy supervisor config for Milvus (monitoring/restart only, autostart=false)
  ...
  echo "Waiting for Milvus to initialize (up to 60 seconds)..."
  for i in {1..60}; do
    if curl -s http://127.0.0.1:9091/healthz >/dev/null 2>&1; then
...
if ! getent hosts | grep -q milvus && curl -s http://127.0.0.1:9091/healthz >/dev/null 2>&1; then
